...
type MountainInputProps = {methods: ReturnType<typeof usTrailheadForm>};
export default function MountainInput({methods}: MountainInputProps) {
  const {mtList, isLoading, refetch} = GetMtList();
  const isMdUp = useResponsive('up', 'md');

  if (isLoading && !mtList) {
    return <MountainInputSkeleton isMdUp={!!isMdUp} />;
  }
      ...
      <Grid item xs={12} sm={12}>
        <div className={css({display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 5})}>
          ...
          <div style={{marginRight: 5}}>
            <RelationJumpButton label="山を登録する" link={`${paths.mountain.create.path}`} />
          </div>
        </div>
        <div style={{margin: '1rem 0.3rem 0 0.3rem', fontSize: '0.8rem'}}>
          登録済みの山と関連付けることができます。
          <div>山が未登録で選択できない場合は、山の登録時に「登山口」で選択してください。後から編集で追加することも可能です。</div>
        </div>
...
const MountainInputSkeleton = ({isMdUp}: {isMdUp: boolean}) => (
