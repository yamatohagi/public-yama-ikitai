...
type MtFacilityInputProps = {methods: ReturnType<typeof useMountainForm>};
      ...
      <Grid item xs={12} sm={12}>
        <div className={css({display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 5})}>
          ...
          <div style={{marginRight: 5}}>
            <RelationJumpButton label="小屋を登録する" link={`${paths.mtFacility.create.path}`} />
          </div>
        </div>
        <div style={{margin: '1rem 0.3rem 0 0.3rem', fontSize: '0.8rem'}}>
          登録済みの山小屋と関連付けることができます。
          <div>山小屋が未登録で選択できない場合は、山小屋の登録時に「登れる山」で選択してください。後から編集で追加することも可能です。</div>
        </div>
