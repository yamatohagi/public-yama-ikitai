...
import {useRemoveQueryParameters} from 'src/hooks/useRemoveQueryParameters';
...
export const PrefectureSearchModal = () => {
  ...
  const {liveState, applyFilters, setLiveValueByKey} = useMtFilterStore();
  const [[...value], setValue] = liveState('area');
  const {removeQueryParameters} = useRemoveQueryParameters();
  const inputValue = router.query.area?.toString();
  ...
  // キーワードでリンクで飛んで来れるように
  useEffect(() => {
    if (!router.isReady || !inputValue) return;
    setValue([inputValue], [inputValue]);
    applyFilters();
    removeQueryParameters(['area']);
  }, [router]);
